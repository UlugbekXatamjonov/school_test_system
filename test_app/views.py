...
class CategoryViewset(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
            ...
            if user.is_authenticated:
                try:
                    subcategory = Sub_Category.objects.filter(id=user.student_tests.id).values()
...
class ResultViewset(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            request_category = int(result_data['category']) # requestdan kelgan category id si
            request_subcategory = int(result_data['subcategory']) # requestdan kelgan subcategory id si
            
            request_tests = json.loads(result_data['test_api']) # requestdan kelgan testlar va belgilangan javoblar json dan asl ma'lumot turiga o'tgazildi
            request_tests = request_tests['tests'] # faqat 'test_id' va 'answer_id' bo'limi ajratib olindi
        ...
        try:
            data_category = Category.objects.filter(id=request_category).values()
            data_subcategory = Sub_Category.objects.filter(id=request_subcategory).values()
            data_category= data_category[0] # bazadan ajratib olingan ma'lumotlar 'list' bo'lagni uchun uni 'index'si bn olib olindi
            data_subcategory = data_subcategory[0] # bazadan ajratib olingan ma'lumotlar 'list' bo'lagni uchun uni 'index'si bn olib olindi
            ...
            for test_id in request_test_ids: # requestdan kelgan test 'id'lari asosida shu 'id'ga mos bazadagi testlar obyekt ko'rinishida ro'yhatga yig'ib olindi 
                ...
                data_tests.append(t[0]) # bazadan ajratib olingan 't' list bo'lgani uchun 0-'index'dagi qiymat olinyapdi
            ...
            for answer_id in request_answer_ids: # requestdan kelgan 'answer' 'id'lari asosida shu 'id'ga mos bazadagi 'answer'lar obyekt ko'rinishida ro'yhatga yig'ib olindi 
                ...
                data_answers.append(a[0]) # bazadan ajratib olingan 'a' list bo'lgani uchun 0-'index'dagi qiymat olinyapdi
        except:
        ...
        try:
            ...
            total_hulosa = "Hulosa mavjud emas!" # O'quvchi tomonidan belgilangan javoblardan yig'ilgan ummumiy 'ball'ga ko'ra belgilangan 'hulosa'
            ...
            if data_subcategory['question_type'] == 'ota': # <O'quvchi tepmeramentini aniqlash>
                total_ball = 0
                total_hulosa = data_answers[0]['question_result'] # Belgilangan yagona javob asosida, shu javob ostidagi 'question_result' olinadi va hulosaga uzatiladi
            ...
            if data_subcategory['question_type'] == 'kta': #  <Kasb tiplarini aniqlash>
                ...
                for test_couple in request_tests: 
                        ...
                        data_test = Question.objects.filter(id=test_couple['test_id']).values().first()
                        data_answer = Answer.objects.filter(id=test_couple['answer_id']).values().first()
                ...
                max_value = max(job_balls.values()) # 'job_balls' lug'atidan eng yuqori valuesi bor qiymatlarni ajratib olamiz
